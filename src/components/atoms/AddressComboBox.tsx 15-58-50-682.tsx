...
export const AddressComboBox = ({
  ...
  useEffect(() => {
    ...
    loader.importLibrary("places").then((places) => {
      setAutocomplete(new places.AutocompleteSuggestion());
  ...
  const fetchAddressList = useCallback(
    debounce(async (query: string) => {
      ...
      if (!autocomplete || !sessionToken) return;
      ...
      // The new API likely returns a Promise
      try {
        console.log("request", request);
        const response = await autocomplete.getSuggestions(request);
        console.log("response", response.predictions);
        // response.predictions is the array of predictions

        const autocompleteSuggestions = response.suggestions.map(
          (suggestion: any) => ({
            id: suggestion.placeId,
            address: {
              string: suggestion.formattedSuggestion,
            },
          })
        );
